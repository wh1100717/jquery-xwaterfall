define(function(require, exports, moudle) {
    ...
    fn = {
        ...
        init: function(container,child){
            var container = $(container);
                fn.initFall(container,child);
                fn.go(container,child);
                this.unloadNum = 0
        },
        go : function(target,child){
            ...
            var i = 0;
            loadFn = function(){
                var winTop = win.scrollTop() + win.height(), absTop = $(target).offset().top +  $(target).height();
                if( winTop >= (absTop-10) && fn.options.scrollSwitch && fn.unloadNum < 3){
                    loading.show()
                    if(ajax){
                        fn.options.scrollSwitch = false;
                        $.get(ajax, {page: page}, function(result){
                            if(result){
                                page++;
    ...
    exports.init = function(container,child){
        fn.init(container,child);
    }
})